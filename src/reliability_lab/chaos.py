...
def _scenario_passed(scenario: ScenarioConfig, result: RunMetrics) -> bool:
    """Named pass/fail criteria per scenario — see reports/final_report.md for rationale."""
    if scenario.name == "primary_timeout_100":
        # Primary is fully down: nearly all traffic must succeed via fallback.
        # Threshold is 0.85 rather than a stricter bar because fallback_success_rate's
        # denominator (fallback_successes + static_fallbacks) excludes cache hits, so the
        # effective sample is small (~35-45 of 100 requests once caching kicks in) — at
        # backup's 5% fail rate a stricter cutoff flags statistical noise as a failure.
        return result.fallback_success_rate > 0.85
    if scenario.name == "primary_flaky_50":
        # Primary oscillates: overall availability should still stay high thanks to fallback.
    ...
    if scenario.name == "both_providers_down":
        # Total outage: system must degrade gracefully (static fallback), not crash.
        denom = result.static_fallbacks + result.fallback_successes
        static_ratio = result.static_fallbacks / denom if denom else 0.0
        return static_ratio > 0.9
